...
def validate(net, val_dataloader,epoch,save_change_map_dir,save_roc_dir):
    ...
    for batch_idx, batch in enumerate(val_dataloader):
        ...
        with torch.no_grad():
            inputs1,inputs2,targets = Variable(inputs1),Variable(input2) ,Variable(targets) #################
      ...
      for batch_idx, batch in enumerate(train_loader):
             ...
             if (batch_idx) % 20 == 0:
                print("Epoch [%d/%d] Loss: %.4f Mask_Loss_conv5: %.4f Mask_Loss_fc: %.4f "
                      "Mask_Loss_embedding: %.4f" % (epoch, batch_idx,loss.item(),contractive_loss_conv5.item(),contractive_loss_fc.item(),contractive_loss_embedding.item()))   ######
